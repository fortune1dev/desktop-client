...
void BuiltinWalletd::run()
{
    ...
    if (Settings::instance().getBytecoindConnectionMethod() != ConnectionMethod::BUILTIN)
        args << QString{"--fortune1coind-bind-address=%1"}.arg(Settings::instance().getBytecoindEndPoint());
...
QString BuiltinWalletd::errorMessage(ReturnCodes err)
{
    ...
    case ReturnCodes::BYTECOIND_ALREADY_RUNNING:
        msg = tr("Cannot run fortune1coind. Another instance of fortune1coind is running.");
        break;
    ...
    case ReturnCodes::BYTECOIND_BIND_PORT_IN_USE:
        msg = tr("Cannot run fortune1coind. Fortune1coind bind port in use.");
        break;
    case ReturnCodes::BYTECOIND_WRONG_ARGS:
        msg = tr("Wrong arguments passed to fortune1coind.");
        break;
    ...
    case ReturnCodes::WALLET_FILE_EXISTS:
        msg = tr("The specified wallet file already exists. Fortune1coind wallet never overwrites an existed wallet file for safety reason. If you want to overwrite the file please remove it manually and try again\n(") + Settings::instance().getWalletFile() + ").";
        break;
