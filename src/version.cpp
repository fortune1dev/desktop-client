...
int compareVersion(const QString& newVersion, const QString& currentVersion)
{
    ...
    for (int i = 0; result == 0 && i < newSubverCount && i < currSubverCount; ++i)
    {
        ...
        QString new_cur_version = "";
        for (int j = 0; j < currentVersions[i].size() ; ++j) {
            if (currentVersions[i].at(j).isDigit())
                new_cur_version += currentVersions[i].at(j);
            else break;
        }
        const int currV = new_cur_version.toInt(&ok);
        if (!ok)
            return 1;
