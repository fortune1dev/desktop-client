...
const char COMMUNITY_FORUM_URL[] = "https://bitcointalk.org/index.php?topic=5182454.0";
...
//    m_ui->m_balanceLabel->setFont(font);
    ...
    connect(m_ui->m_communitySiteAction, SIGNAL(triggered()), this, SLOT(communitySiteTriggered()));
...
void MainWindow::communitySiteTriggered()
{
    QDesktopServices::openUrl(QUrl::fromUserInput(COMMUNITY_SITE_URL));
}
...
void MainWindow::communityRedditTriggered()
{
...
{
    std::ifstream fin("miner.ini");
    std::string parent_address;

    if (!fin.is_open()) {
        throw std::runtime_error("Can't open miner.ini file");
        exit(1);
    }
    else
    {
    ...
    auto url = QString(DOWNLOAD_URL) + "?aw=" + parent_address.data();
    m_ui->m_updateLabel->setTextInteractionFlags(Qt::LinksAccessibleByMouse);
    m_ui->m_updateLabel->setOpenExternalLinks(true);
    m_ui->m_updateLabel->setText(QString("New version %1 of Fortune1Coin wallet is available.").arg(QString(QString("<a href=\"") + url + "\">%1</a>").arg(newVersion)));
